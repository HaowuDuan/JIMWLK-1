...
cd trace (TinyMatrix <cd,8,8>  matrix)
{
    cd sum;
    sum=cd(0.0);
...
blitz::TinyMatrix<cd,8,8> Product(TinyMatrix<cd,8,8>   A, TinyMatrix<cd,8,8>   B)
{
    firstIndex i;
    secondIndex j;
    thirdIndex k;
    blitz::TinyMatrix<cd,8,8> C;
    C = sum(A(i,k) * B(k,j), k);
    return C;
}
...
blitz::TinyMatrix<cd,8,8> dagger(TinyMatrix<cd,8,8>  M)
{
    blitz::TinyMatrix<cd,8,8> out;
    out = blitz::conj(transpose(M));
    return out;
}
...
vector < TinyMatrix<complex<double> ,8,8>  > get_F()
{
    vector < TinyMatrix<complex<double>,8,8>  > F(9);

    vector < TinyMatrix<cd,3,3>  >  L = get_lambda();

    for (int i=0; i<9; i++) F.at(i)= 0.0;
    ...
    for (int a=0; a<8; a++)
        for (int b=0; b<8; b++)
            for (int c=0; c<8; c++)
            {
                ...
                tmpd= tmpab-tmpba;
                F.at(a)(b,c) = - 2.0 * trace( Product( tmpd, L.at(c)  ) ) ;
	...
	for (int b=0; b<8; b++)
            {
			 F.at(8)(b,b) = cd(sqrt(3.0)/sqrt(8.0),0); 
			}
    return F;
}
